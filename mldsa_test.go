...
func TestExternalMu(t *testing.T) {
	testAllParameters(t, func(t *testing.T, params Parameters) {
		...
		for _, ctx := range []string{"", "ctx"} {
			...
			sig, err := sk.Sign(nil, μ, CryptoMLDSAMu)
			if err != nil {
			...
			detSig, err := sk.SignDeterministic(μ, CryptoMLDSAMu)
			if err != nil {
			...
			detSig2, err := sk.SignDeterministic(μ, CryptoMLDSAMu)
			if err != nil {
		...
		sigA, err := sk.Sign(nil, μA, CryptoMLDSAMu)
		if err != nil {
		...
		if _, err := sk.Sign(nil, make([]byte, 32), CryptoMLDSAMu); err == nil {
			t.Errorf("Sign(MLDSAMu) with 32-byte input: want error, got nil")
		}
		if _, err := sk.SignDeterministic(make([]byte, 32), CryptoMLDSAMu); err == nil {
...
func (muOptions) HashFunc() crypto.Hash { return CryptoMLDSAMu }
